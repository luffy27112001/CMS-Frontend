...
const MyCourseCard = ({ course }) => {
  ...
  return (
    <Link to={`/me/course/${course._id}`} className="mx-5 my-3">
      <img src={course.courseImages[0].url} alt={course.courseName} className="w-80 h-48 transition ease-in-out hover:-translate-y-3" />
      <p className="text-xl font-semibold mt-2">{course.courseName}</p>
      <span className="font-semibold">By <span className="text-blue-500">{course.author}</span></span>
      ...
      <div className="flex items-center text-xl mt-2">
        ...
        <span className="relative left-2">({course.enrolledCount} Students)</span>
      </div>
