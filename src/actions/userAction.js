...
export const enrolledCourse = (courseId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(`/api/v1/me/course/${courseId}`);
...
// Created Course
export const createdCourse = () => async (dispatch) => {
  try {
    dispatch({ type: CREATED_COURSE_REQUEST });

    const { data } = await axios.post(`/api/v1/me/courses`);

    dispatch({
      type: CREATED_COURSE_SUCCESS,
      payload: data.success,
    });
  } catch (error) {
    dispatch({
      type: CREATED_COURSE_FAIL,
      payload: error.response.data.message,
    });
  }
};
